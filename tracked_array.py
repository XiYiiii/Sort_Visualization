...
class TrackedArray:
    def __init__(self, array, _history=None, _path=()):
        ...
        for i, item in enumerate(array):
            if isinstance(item, list):
                self._array.append(
                    TrackedArray(item, _history=self.history, _path=self._path + (i,))
                )
            else:
    ...
    def __getitem__(self, key):
        ...
        if not isinstance(item, TrackedArray):
            ...
            self.history.append(
                {"array": self._get_raw_copy(), "highlights": highlights}
            )
        return item
    ...
    def __setitem__(self, key, value):
        ...
        if isinstance(value, list):
            ...
            self._array[key] = TrackedArray(
                value, _history=self.history, _path=new_path
            )
        else:
    ...
    def append(self, value):
        ...
        if isinstance(value, list):
            ...
            self._array.append(
                TrackedArray(value, _history=self.history, _path=new_path)
            )
        else:
